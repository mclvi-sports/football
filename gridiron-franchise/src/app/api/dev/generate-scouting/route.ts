...
import {
  generateScouting,
  getScoutingStats,
} from '@/lib/scouting/scouting-generator';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const scouting = generateScouting(teamTiers);
    const stats = getScoutingStats(scouting);
    ...
    console.error('Scouting generation error:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to generate scouting department' },
      { status: 500 }
    );
  }
}
