...
function formatAttribute(attr: string): string {
  return attr.replace(/([A-Z])/g, ' $1').replace(/^./, (str) => str.toUpperCase()).trim();
}
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      ...
      <div className="flex-1 h-2 bg-muted rounded overflow-hidden flex">
        <div className="h-full bg-blue-500" style={{ width: `${pass}%` }} title={`${pass}% pass`} />
        <div className="h-full bg-green-500" style={{ width: `${run}%` }} title={`${run}% run`} />
      </div>
  ...
  return (
    <div className="bg-card border rounded-lg p-4">
      ...
      <div className="mb-4">
        ...
        <div className="space-y-1">
          <TendencyBar pass={scheme.playCallingTendencies.first_and_10.pass} run={scheme.playCallingTendencies.first_and_10.run} label="1st & 10" />
          <TendencyBar pass={scheme.playCallingTendencies.third_and_long.pass} run={scheme.playCallingTendencies.third_and_long.run} label="3rd & Long" />
          <TendencyBar pass={scheme.playCallingTendencies.red_zone.pass} run={scheme.playCallingTendencies.red_zone.run} label="Red Zone" />
        </div>
      </div>
      ...
      <div className="flex gap-4 text-xs">
        <div>
          ...
          {scheme.strongAgainst.length > 0 ? scheme.strongAgainst.map((s) => DEFENSIVE_SCHEMES[s].name).join(', ') : 'None'}
        </div>
        <div>
          <span className="text-red-400 font-semibold">Weak vs: </span>
          {scheme.weakAgainst.length > 0 ? scheme.weakAgainst.map((s) => DEFENSIVE_SCHEMES[s].name).join(', ') : 'None'}
        </div>
      </div>

      {/* Expandable Personnel */}
      <button onClick={() => setExpanded(!expanded)} className="mt-4 text-xs text-muted-foreground hover:text-foreground transition-colors">
      ...
      {expanded && (
        <div className="mt-2 p-2 bg-muted/50 rounded text-xs space-y-1">
          {scheme.idealPersonnel.map((p, i) => (
            <div key={i}>
              <span className="font-semibold">{p.position}:</span> {p.idealArchetypes.join(', ')}
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div className="bg-card border rounded-lg p-4">
      ...
      <div className="flex gap-4 text-xs">
        <div>
          <span className="text-green-400 font-semibold">Strong vs: </span>
          {scheme.strongAgainst.length > 0 ? scheme.strongAgainst.map((s) => OFFENSIVE_SCHEMES[s].name).join(', ') : 'None'}
        </div>
        <div>
          <span className="text-red-400 font-semibold">Weak vs: </span>
          {scheme.weakAgainst.length > 0 ? scheme.weakAgainst.map((s) => OFFENSIVE_SCHEMES[s].name).join(', ') : 'None'}
        </div>
      </div>
          ...
          <div className="space-y-1">
            {philosophy.bonuses.map((bonus, i) => (
              <div key={i} className="text-xs">
                <span>{bonus.effect}:</span> <span className="text-green-400">{bonus.value}</span>
              </div>
            ))}
          </div>
        </div>
        <div>
          ...
          <div className="space-y-1">
            {philosophy.penalties.map((penalty, i) => (
              <div key={i} className="text-xs">
                <span>{penalty.effect}:</span> <span className="text-red-400">{penalty.value}</span>
              </div>
            ))}
          </div>
        </div>
...
function MatchupMatrix() {
  const [selectedCell, setSelectedCell] = useState<{ offense: OffensiveScheme; defense: DefensiveScheme } | null>(null);
        ...
        <table className="text-xs border-collapse">
          ...
          <tbody>
            {ALL_OFFENSIVE_SCHEMES.map((off) => (
              <tr key={off}>
                <td className="p-2 border bg-muted font-semibold">{OFFENSIVE_SCHEMES[off].name}</td>
                {ALL_DEFENSIVE_SCHEMES.map((def) => {
                  ...
                  return (
                    <td
                      ...
                      className={cn(
                        ...
                        result === 'advantage' ? 'bg-green-500/30 hover:bg-green-500/50' :
                        result === 'disadvantage' ? 'bg-red-500/30 hover:bg-red-500/50' :
                        'bg-muted/50 hover:bg-muted',
                        selectedCell?.offense === off && selectedCell?.defense === def ? 'ring-2 ring-primary' : ''
                      )}
      ...
      {selectedCell && (
        <div className="mt-4 p-4 bg-card border rounded-lg">
          <h4 className="font-bold mb-2">
            {OFFENSIVE_SCHEMES[selectedCell.offense].name} vs {DEFENSIVE_SCHEMES[selectedCell.defense].name}
          </h4>
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <p className="font-semibold text-blue-400">Offensive Scheme</p>
              <p className="text-muted-foreground">{OFFENSIVE_SCHEMES[selectedCell.offense].philosophy}</p>
              <p className="mt-2">
            ...
            <div>
              ...
              <p className="text-muted-foreground">{DEFENSIVE_SCHEMES[selectedCell.defense].baseFormation}</p>
              <p className="mt-2">
                <span className="text-green-400">Strong vs: </span>
...
export function SchemesView({ mode, maxHeight = mode === 'embedded' ? '500px' : undefined }: SchemesViewProps) {
      ...
      {activeTab === 'offensive' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-muted-foreground">{ALL_OFFENSIVE_SCHEMES.length} schemes</span>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            {ALL_OFFENSIVE_SCHEMES.map((schemeId) => (
              <OffensiveSchemeCard key={schemeId} scheme={OFFENSIVE_SCHEMES[schemeId]} />
            ))}
          </div>
        </div>
      )}
      ...
      {activeTab === 'defensive' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-muted-foreground">{ALL_DEFENSIVE_SCHEMES.length} schemes</span>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            {ALL_DEFENSIVE_SCHEMES.map((schemeId) => (
              <DefensiveSchemeCard key={schemeId} scheme={DEFENSIVE_SCHEMES[schemeId]} />
            ))}
          </div>
        </div>
      )}
      ...
      {activeTab === 'special_teams' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-muted-foreground">{ALL_ST_PHILOSOPHIES.length} philosophies</span>
          </div>
          <div className="grid gap-4 md:grid-cols-3">
            {ALL_ST_PHILOSOPHIES.map((philosophyId) => (
              <STPhilosophyCard key={philosophyId} philosophy={ST_PHILOSOPHIES[philosophyId]} />
            ))}
          </div>
        </div>
      )}
