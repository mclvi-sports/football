...
import { storeScouting, getScouting, getCachedStats } from '@/lib/scouting/scouting-store';
...
interface ScoutingDeptViewProps {
...
export function ScoutingDeptView({
  ...
  useEffect(() => {
    const existingScouting = getScouting();
    const existingStats = getCachedStats();
    if (existingScouting) {
      setScouting(existingScouting);
      if (existingStats) {
                ...
                <div className="space-y-2">
                  {stats.topDepartments.map((d, i) => (
                    <div key={d.teamId} className="flex justify-between items-center">
                      <span className="text-sm">{i + 1}. {d.teamId}</span>
              ...
              <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4">
                ...
                <div className="space-y-2">
                  {stats.bottomDepartments.map((d, i) => (
                    <div key={d.teamId} className="flex justify-between items-center">
                      <span className="text-sm">{32 - 4 + i}. {d.teamId}</span>
              ...
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(stats.positionExpertiseDistribution)
                  ...
                  .map(([expertise, count]) => (
                    <div
                      key={expertise}
                      className={cn('rounded-lg p-3 flex items-center justify-between border', EXPERTISE_COLORS[expertise as PositionExpertise])}
                    >
                      <span className="text-sm">{POSITION_EXPERTISE[expertise as PositionExpertise].name}</span>
              ...
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(stats.regionalExpertiseDistribution)
                  ...
                  .map(([region, count]) => (
                    <div
                      key={region}
                      className={cn('rounded-lg p-3 flex items-center justify-between border', REGION_COLORS[region as RegionalExpertise])}
                    >
                      <span className="text-sm">{REGIONAL_EXPERTISE[region as RegionalExpertise].name}</span>
                      <span className="font-bold">{count}</span>
                    </div>
                  ))}
              </div>
