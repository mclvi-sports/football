...
export function StatsView({ games, standings, onPlayerClick, onTeamClick }: StatsViewProps) {
  const [statCategory, setStatCategory] = useState<'passing' | 'rushing' | 'receiving' | 'defense' | 'team'>('passing');
  ...
  const renderPlayerRow = (
    ...
    <div
      ...
      style={{
        gridTemplateColumns: type === 'passing' ? '2rem 1fr 3rem 4rem 4rem 4rem 3rem 3rem 4rem' :
                             type === 'rushing' ? '2rem 1fr 3rem 3rem 4rem 3rem 4rem' :
                             type === 'receiving' ? '2rem 1fr 3rem 4rem 4rem 4rem 3rem' :
    ...
    >
      ...
      {type === 'passing' && (
        <>
          <div className="text-center">{player.passing.completions}/{player.passing.attempts}</div>
      ...
      {type === 'receiving' && (
        <>
          <div className="text-center">{player.receiving.catches}/{player.receiving.targets}</div>
      ...
      <Tabs defaultValue="passing" className="w-full">
          ...
          <Card>
            ...
            <CardContent className="p-0">
              <div className="rounded-b-lg border border-zinc-800 overflow-hidden">
                ...
                {topPassers.length === 0 ? (
                  <div className="p-4 text-center text-zinc-500">No passing stats available yet</div>
                ) : (
          ...
          <Card>
            ...
            <CardContent className="p-0">
              <div className="rounded-b-lg border border-zinc-800 overflow-hidden">
                ...
                {topRushers.length === 0 ? (
                  <div className="p-4 text-center text-zinc-500">No rushing stats available yet</div>
                ) : (
            ...
            <CardContent className="p-0">
              <div className="rounded-b-lg border border-zinc-800 overflow-hidden">
                ...
                {topReceivers.length === 0 ? (
                  <div className="p-4 text-center text-zinc-500">No receiving stats available yet</div>
                ) : (
            ...
            <CardContent className="p-0">
              <div className="rounded-b-lg border border-zinc-800 overflow-hidden">
                ...
                {topDefenders.length === 0 ? (
                  <div className="p-4 text-center text-zinc-500">No defensive stats available yet</div>
                ) : (
        ...
        <TabsContent value="team" className="mt-4 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Card>
              ...
              <CardContent className="space-y-1">
                {teamOffenseStats.map((team, i) => {
                  ...
                  const ppg = gamesPlayed > 0 ? (team.record.pointsFor / gamesPlayed).toFixed(1) : '0.0';
                  return (
                    <div
                      key={team.teamId}
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm text-blue-400">Best Defense (PPG Allowed)</CardTitle>
              </CardHeader>
              <CardContent className="space-y-1">
                {teamDefenseStats.map((team, i) => {
                  const gamesPlayed = team.record.wins + team.record.losses + team.record.ties;
                  const ppga = gamesPlayed > 0 ? (team.record.pointsAgainst / gamesPlayed).toFixed(1) : '0.0';
                  return (
                    <div
                      key={team.teamId}
                      onClick={() => onTeamClick?.(team.teamId)}
